...
對於每一筆測試資料，若你的程式在函式 `encode()` 中所回傳的字串長度為 $x$，定義 $A, y, z$ 與函數 $f$ 為:
...
$$
f(q) = \frac{1}{1 + 114514^{-(q-0.67)}}
$$

若你的函式 `query()` 正確了回答所有 $Q$ 筆詢問，則你將得到分數比重 $W$ :
...
\begin{aligned}
    & 1 \text{ if } x \le A \\
    & \frac{1}{2} + \frac{1}{2} \cdot \frac{f(z) - f(0)}{f(1) - f(0)} \text{ if } A < x \le B \\
    & 0 \text{ if } x > B
\end{aligned}
\right.
$$

本題共有五組子任務，條件限制如下所示。每一組可有一或多筆測試資料，

 - \textbf{對於子任務 $5$ -- 無額外限制 : } 你在該子任務的得分為所有測試資料中分數比重 $W$ 的最小值，乘以該子任務的分數。
 - \textbf{對於其餘子任務 : } 你在該子任務的得分將僅依據是否正確答對所有測試資料中的 $Q$ 筆詢問，而給予該子任務的分數。
