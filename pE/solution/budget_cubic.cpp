...
constexpr long long OPERATION_BUDGET = 2000000;
int decodedN;
...
std::vector<int> budgetedCuts(const int n,
    const Matrix& w) {
    ...
    for (int length = 2; length <= n; ++length) {
        for (int l = 0; l + length <= n; ++l) {
            ...
            int low = l;
            int high = r - 1;
            if (operations + length - 1 > OPERATION_BUDGET) {
                low = std::max(opt[id(n, l, r - 1)], l);
                high = std::min(opt[id(n, l + 1, r)], r - 1);
            ...
            if (operations + length - 1 <= OPERATION_BUDGET) {
                ...
                const int candidates[3] = {low, (low + high) / 2, high};
...
std::string encode(int n, const Matrix& w) {
    return encodeCuts(n, budgetedCuts(n, w));
}
